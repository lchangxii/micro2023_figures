...
def process_sheet_name(sheet_name,benchname,x_loc,y_loc):
    ...
    atomic_regions = [(sheet_name,'walltime'), (sheet_name,'bbwalltime'), (sheet_name,'wfwalltime'),(sheet_name,'mixwalltime')]

    atomic_regions_name =  ["MGPUSim",'BB-Sampling',"Warp-Sampling","Photon"]
    dicts = []
    ...
    atomic_regions = [(sheet_name,'simtime'), (sheet_name,'bbsimtime'), (sheet_name,'wfsimtime'),(sheet_name,'mixsimtime')]
    ...
    colors = [ "#3264a6","none",'#a32a30',"none"]
    hatchcolors = ['#20416b','#a32a30', '#a32a30',"#a32a30"]
    ...
    bb_speedup=np.divide(np.array(full_data),np.array(bb_data))
    max_bbspeedup=max(bb_speedup)

    wf_speedup=np.divide(np.array(full_data),np.array(wf_data))
    max_wfspeedup=max(wf_speedup)
    print(wf_data)
    print(bb_data)
    print("maxspeed",max_speedup,max_bbspeedup,max_wfspeedup)

    ax2.set_ylabel('Wall Time (s)',fontsize = global_fontsize)
    idx=0
    print(dictsimtimes)
    colors = [ "#3264a6","#a32a30",'#a32a30',"#a32a30"]
    hatchs = ["x","v","^","o"]
    ...
    bbaccuracy = np.divide( np.fabs(bb_data-full_data),full_data)
    wfaccuracy = np.divide( np.fabs(wf_data-full_data),full_data)
    ...
    ax1.legend(hd,lab,fontsize = legend_fontsize,bbox_to_anchor=(-0.03,1),loc="lower left",ncol=len(dicts),columnspacing=legend_space)
    #plt.text(x_loc,y_loc,"%s (%.2fX,%.2fX,%.2fX)"%(benchname,max_bbspeedup,max_wfspeedup,max_speedup),fontsize=global_fontsize*1.5)
    name = __file__+sheet_name+"_accuracy"
    ...
    #plt.show()
    return list(accuracy),list(bb_speedup),list(wf_speedup),list(speedups),list(bbaccuracy),list(wfaccuracy)
#average_accuracy=10.02
benchnames=[     "SC",  "ReLU","MM",    "AES","SPMV","FIR"]
sheet_names=["conv","relu","matmul","aes","spmv","fir"]
text_loc=[(0,1000), (0,600),(0,5000),(0,1200),(0,20000),(0,1000)]
...
for idx in range(len(sheet_names)):
    ...
    locs=text_loc[idx]
    data,bb_speedup,wf_speedup,speedups,bbaccracy,wfaccuracy=process_sheet_name( sheet_name,benchname,locs[0],locs[1])
    all_accuracy+=data
    all_speedup+=speedups
    all_bbspeedup+=bb_speedup
    all_wfspeedup+=wf_speedup
    all_wfaccuracy+=wfaccuracy
    all_bbaccuracy+=bbaccracy
average=np.average(all_accuracy)
bbaverage=np.average(all_bbaccuracy)
wfaverage=np.average(all_wfaccuracy)

from statistics import geometric_mean
geoaverage=geometric_mean(all_speedup)
print("average",average)
print("average",bbaverage)
print("average",wfaverage)
print("speedup avg",geoaverage)
bbgeoaverage=geometric_mean(all_bbspeedup)
print("bbspeedup avg",bbgeoaverage)
wfgeoaverage=geometric_mean(all_wfspeedup)
print("wfspeedup avg",wfgeoaverage)