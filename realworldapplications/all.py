...
def process_sheet_name(sheet_name,benchname,x_loc,y_loc):
    ...
    fig,ax1=plt.subplots(figsize=(10, 3.5))#1, 1, 1)
    ax2=ax1.twinx()
    makers = ['.','^','*','x','|','1','H','S']
    ...
    yticks=range(0,400000,100000)
    #yticks_txt=["1 hour","1 day"]
    #yticks=[3600,3600*24]
    ax2.set_yticks(yticks,yticks,fontsize = global_fontsize)
    ...
    ax1.set_ylabel('Sim Time (ms)',fontsize = global_fontsize)
    yticks=range(0,200,50)
    ax1.set_yticks(yticks,yticks,fontsize = global_fontsize)
    ...
    plt.text(x_loc,y_loc,"%s (%.1f"%(benchname,max_speedup)+r"$\times$)",fontsize=global_fontsize*1.5)
    name = "realapplications"
    ...
    ax1.set_xticks(index, benchmarks,fontsize = global_fontsize, rotation=45,ha='right', rotation_mode='anchor' )#rotation=90,
...
average=np.average(average_accuracy)
